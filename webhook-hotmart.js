...
function montarNotificacaoAdminHTML({ nome, email, plano, codigo, transacao, valor }) {
    return `
<div style="font-family:Arial,sans-serif; background:#0a0a0a; color:#f5f5f7; padding:24px; border-radius:8px; max-width:560px;">
    ...
    <table style="width:100%; border-collapse:collapse;">
        ...
        <tr><td style="padding:8px 0; color:#a1a1a6;">Código gerado</td><td style="padding:8px 0; font-family:monospace; font-size:12px; color:#d4af37;">${codigo}</td></tr>
    </table>
</div>`;
}
...
function makeHandler({ resend }) {
    ...
    return async function handler(req, res) {
        try {
            ...
            // 7. Notificação pro admin (não-bloqueante; se falhar, log e segue)
            try {
                await resend.emails.send({
                    from: `EventCalc <${fromEmail}>`,
                    to: adminEmail,
                ...
                console.error('[hotmart-webhook] Falha ao notificar admin (não-bloqueante):', errAdmin.message);
            }

            console.log(`[hotmart-webhook] ✅ Código gerado e enviado | ${buyer.email} | ${plano} | ${codigo.slice(0, -5)}-***`);
            return res.status(200).json({ ok: true, acao: 'codigo_gerado' });
